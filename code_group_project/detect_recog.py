...
yynet = YuNet(modelPath='/home/medievalvn/Documents/USTH_project/group_project/facenet-pytorch-glint360k-master/model/face_detect_yunet/face_detection_yunet_2023mar.onnx',
              inputSize=[320, 320],
              confThreshold=0.9,
              nmsThreshold=0.3,
              topK=5000,
...
while True:
    ...
    if faces.shape[0] > 0:
        for i in range(faces.shape[0]):
            ...
            with open(f"realtime_faces/face_vector_{i}.txt", "w") as file:
                for value in embedding.flatten():
                    file.write(str(value) + "\n")

            print(f"Face vector for face {i} saved to face_vector_{i}.txt")
    else:
